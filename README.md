...
# 2. Compile (4 passes for correct refs/citations) from the repo root
~/.local/bin/tectonic main_v2.tex && cd ..
#   or: latexmk -pdf main_v2.tex   (pdflatex + bibtex)
...
pdftotext -layout main_v2.pdf /tmp/paper.txt \
...
> confirm `main_v2.pdf` still fits (0 overfull, 0 undefined).
...
| `main_v2.tex` | **The canonical FIRE 2026 camera-ready paper entry** (ACM `acmart` `sigconf`, 2-column, author block with Bose / Tripathi / Chatterjee† at KIIT, conference metadata) — compiled from the repo root with tectonic; regenerated by `md2tex.py` |
| `latex/` | Generated LaTeX pieces — `sections/sec01..06.tex` + `paper_references.bib` copy + `_validate_tex.py` / `_pagemap.py`; compiles clean via `main_v2.tex`: content fills the full 9 pp limit (refs start p10, refs excluded from the count), 9 tables + **3 embedded figures**, zero undefined refs/citations/overfull; `main_v2.pdf` built |
| `md2tex.py` | Build tool — converts `paper_full_draft.md` programmatically (headings, tables, math, `\Cref`/`\citep` wiring, markdown images `![caption](path){#label}` → `figure*` envs, drafting metadata → comments, camera-ready trims via `_trims.py`) and writes `main_v2.tex` (repo root) + `latex/sections/*.tex`; re-run after editing the markdown |
| `_trims.py` | Camera-ready prose-trim table used by `md2tex.py` — 102 exact-match sentence tightenings (redundant prose only; every headline number, citation, and cross-reference preserved; chained trims rewrite text that an earlier trim in the chain produced); `md2tex.py` reports applied/never-applied (re-run `md2tex.py` after editing any draft and check the trims report — re-assembling `paper_full_draft.md` can invalidate exact-match trims) |
...
| `latex/_pagemap.py` | Page-break reporter — label → PDF page from `main_v2.aux` (repo root) + per-page fill from `pdftotext` (pass the extracted text file as arg) |
