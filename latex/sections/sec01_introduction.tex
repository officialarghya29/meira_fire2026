...
We build \textbf{MEIRA}, a Memory-Enhanced Interpretable Retrieval Agent coupling (i) a \textbf{64-slot episodic memory bank} that persists retrieval states across turns, (ii) a \textbf{temporal-decay mechanism} that down-weights stale memories, and (iii) an \textbf{XAI attribution head} that emits a normalised confidence per retrieved document. To measure these we introduce \textbf{XAIR@K} (nDCG@K interpolated with mean attribution confidence over relevant top-K hits, $w = 0.25$) and \textbf{MDS} (fraction of the memory bank actually used), and evaluate every system on two synthetic-but-realistic FIRE-style benchmarks (\Cref{sec:datasets-benchmarks}) under a fully specified ten-seed protocol (\Crefrange{sec:datasets-protocol}{sec:datasets-stats}).
...
Across ten evaluation seeds, MEIRA-full is the best model on every ranking metric on both datasets: F1 = 0.826$\pm$0.015 / 0.780$\pm$0.030 vs 0.740$\pm$0.013 / 0.680$\pm$0.029 for the strongest baseline (+11.7\% / +14.6\% relative), significant at $p < 0.0001$ ($t = 20.120$ / 13.058 on nDCG@10) under both multiplicity corrections at every threshold --- no comparison involving MEIRA-full is ever lost. The ablation ranks the components as \textbf{memory > decay > XAI} ($\Delta$F1 +0.110/+0.124, +0.078/+0.086, +0.049/+0.054), the XAI head uniquely responsible for XAIR@10 ($\Delta$XAIR@10 +0.894/+0.886) and the memory bank at full utilisation (MDS = 1.000$\pm$0.000). Two caveats, developed below: at shallow cutoffs (P@5 / P@10) the models are effectively tied, so the claim is superiority in ranking quality rather than raw precision; and the only fragile comparisons (BM25 vs TF-IDF; MEIRA-no-decay vs ColBERT-like) never involve MEIRA-full.
